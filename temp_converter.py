...
class UnitConverterInterface(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.tc2_combo = wx.ComboBox(panel, choices=["°C", "°F", "K"], style=wx.CB_READONLY)
        self.tc3_combo = wx.ComboBox(panel, choices=["°C", "°F", "K"], style=wx.CB_READONLY)

        fgs.AddMany([(temp_value), (self.tc1, 1, wx.EXPAND), (from_temp), 
        (self.tc2_combo, 1, wx.EXPAND), (to_temp, 1, wx.EXPAND), (self.tc3_combo, 1, wx.EXPAND), (calc_button),
    ...
    def on_press_calculate(self, event):
        try:
            self.result.Label = conv.temperature_converter(value=int(self.tc1.GetValue()),
...
def main():
    ...
    app = wx.App()
    frame = UnitConverterInterface(None, style=wx.MINIMIZE_BOX | 
